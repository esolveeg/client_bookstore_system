          ...
          <div id="mid">
            <div class="info">
              ...
              <p>
                ...
                <span v-if="order.customer && order.customer.name"
                  >customer: {{ order.customer.name }}<br
                /></span>
                <span v-if="order.customer && order.customer.email"
            ...
            <div id="table">
              <table>
                ...
                <tr class="tabletitle">
                  ...
                  <td class="payment">
                    <h2 class="hresete">
                      EGP{{ order.total + order.discount }}
                    </h2>
                  </td>
                </tr>
                <tr class="tabletitle" v-if="order.discount">
                  ...
                  <td></td>
                  <td class="Rate">
                    <h2 class="hresete">discount({{ order.discount }})</h2>
                  </td>
                  <td class="payment">
                      ...
                      <table class="highlight responsive-table">
                        ...
                        <tbody>
                          ...
                          <tr class="border-none">
                            <td colspan="4"></td>
                            <td>Sub Total:</td>
                            <td>EGP{{ parseInt(order.sub) }}</td>
                          </tr>
                          <tr class="border-none" v-if="order.discount">
                            <td colspan="4"></td>
                            <td>discount({{ order.discount }}%)</td>
                            <td>{{ parseInt(order.disc_val) }}</td>
                          </tr>
                          <tr class="border-none">
                            <td colspan="4"></td>
...
export default {
  ...
  data() {
    return {
      ...
      order: {},
      btns: [],
      path: [{ title: "order", route: "orders.index" }]
    };
  },
  methods: {
    printData(className) {
    ...
    getOrder() {
      this.$http.get(`ordernew/${this.$route.params.id}`).then(response => {
