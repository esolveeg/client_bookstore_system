...
import { usersQuery, meQuery } from "@/graphql/queries/queries.js";
...
export default {
  ...
  apollo: {
    me: {
      ...
      result({ data: { me } }) {
        ...
        permissions.includes("create_users") ? (this.canAdd = true) : "";
  ...
  methods: {
    ...
    goToView(id) {
      this.$router.push({ name: "users.view", params: { id: parseInt(id) } });
    },
    goToEdit(id) {
      this.$router.push({ name: "users.edit", params: { id: parseInt(id) } });
    },
    setDeleteData(data) {
