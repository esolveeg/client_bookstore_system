...
import { rebatesQuery, meQuery } from "@/graphql/queries/queries.js";
import M from "materialize-css";
export default {
  data() {
    let columns = [
      { label: "cause", name: "cause", dbname: "cause" },
      { label: "value", name: "value", dbname: "value" },
      { label: "employee", name: "employee", dbname: "employee", sub: "name" },
  ...
  apollo: {
    me: {
      ...
      result({ data: { me } }) {
        ...
        permissions.includes("create_rebates") ? (this.canAdd = true) : "";
        permissions.includes("delete_rebates") ? (this.canDelete = true) : "";
        permissions.includes("edit_rebates") ? (this.canEdit = true) : "";
        permissions.includes("read_rebates") ? (this.canView = true) : "";
      }
    },
  ...
  methods: {
    ...
    create() {
      this.$router.push({ name: "rebates.create" });
    },
    goToView(id) {
      this.$router.push({ name: "rebates.view", params: { id: parseInt(id) } });
    },
    goToEdit(id) {
      this.$router.push({ name: "rebates.edit", params: { id: parseInt(id) } });
    },
    setDeleteData(data) {
